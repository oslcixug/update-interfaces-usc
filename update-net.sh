...
# Version mellorada por Antonio Costa para pedir ao usuario os valores das variables
# do Edificio e Vlan.
...
EDIFICIO=$(zenity --entry --text="Introduce o número do edificio:" --title="Oficina de Software Libre")
# Se o usuario pulsa o botón Cancelar, hai que saír do script
if [ $? -ne 0 ] ; then
exit 1

fi
VLAN=$(zenity --entry --text="Introduce o número da VLAN:" --title="Oficina de Software Libre")
# Se o usuario pulsa o botón Cancelar, hai que saír do script
if [ $? -ne 0 ] ; then
exit 1
fi
...
ROUTER=$(zenity --entry --text="Introduce a IP do ROUTER:" --title="Oficina de Software Libre")
# Se o usuario pulsa o botón Cancelar, hai que saír do script
if [ $? -ne 0 ] ; then
exit 1
fi
